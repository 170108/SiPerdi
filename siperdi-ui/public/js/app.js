...
const adminBooks = document.querySelector("[data-admin-books]");
...
const pendingMessage = document.querySelector('[data-message="pending"]');
...
async function loadBookInventory() {
    ...
    const migrate = (list) => {
      ...
      const normalized = list.map((b) => {
        const rawCat = b.kategori ?? b.category ?? b.kategori_buku;
        const cat = normalizeCategory(rawCat);
        // jika data backend pakai "category", kita setel field "kategori" agar konsisten di UI
        if (cat !== b.kategori || !b.kategori) changed = true;
...
const calculateDueDate = (tanggal, durasi) => {
  ...
  const days = Math.min(Number(durasi || 7), 7);
...
const renderPublicCatalog = () => {
  ...
  loadBookInventory()
    .then((books) => {
      ...
      if (!umum.length) {
        umumContainer.innerHTML = '<div class="table-row muted"><span>Tidak ada buku koleksi.</span></div>';
      }
  ...
  if (loanList && loanEmpty) {
    ...
    if (!user) {
      ...
      userLoans.forEach((loan) => {
        ...
        const extendButton = loan.returnStatus !== "approved"
          ? `<button class="btn tiny ghost" data-extend-loan data-loan-id="${loan.id}">Ajukan Perpanjangan +7 hari</button>`
          : "";
        row.innerHTML = `
          ...
          <span class="loan-action">${actionHtml} ${extendButton}</span>
        `;
...
const renderAdminDashboard = (session) => {
  ...
  if (adminBooks) adminBooks.textContent = String(totalBookCount);
...
// Tabel verifikasi siswa (pending) di halaman Admin/verifikasi-siswa.html
const renderPendingVerifications = (session) => {
  if (!pendingList || !pendingEmpty) return;
  const isAdmin = session?.role === "admin";
  const showEmpty = () => {
    pendingList.innerHTML = "";
    pendingEmpty.classList.add("show");
  ...
  const users = getUsers();
  const pending = users.filter((u) => (u.role || "student") === "student" && u.status === "pending");
  if (pendingCount) pendingCount.textContent = String(pending.length);
  if (!pending.length) {
    showEmpty();
    return;
  }
  pendingEmpty.classList.remove("show");
      ...
      (u) => `
        <div class="table-row">
          ...
          <span class="flex gap-8">
            <button class="btn tiny solid" data-approve-student data-email="${u.email}">Setujui</button>
            <button class="btn tiny ghost" data-reject-student data-email="${u.email}">Tolak</button>
          </span>
        </div>
      `
    )
...
const renderAll = () => {
  ...
  renderPendingVerifications(session);
};
...
if (loanList) {
  loanList.addEventListener("click", (event) => {
    ...
    const extendBtn = event.target.closest("[data-extend-loan]");
    const session = getSession();
    if (!session) {
      openModal("login");
      return;
    ...
    if (extendBtn) {
      if (session.role === "admin") {
        setLoanListMessage("Admin tidak dapat memperpanjang pinjaman.", "error");
        return;
      }
      const loanId = extendBtn.dataset.loanId;
      const loan = userLoans.find((item) => item.id === loanId);
      if (!loan) return;
      if (loan.returnStatus === "pending" || loan.returnStatus === "approved") {
        setLoanListMessage("Tidak bisa diperpanjang, pengembalian sudah diajukan/disetujui.", "error");
        return;
      }
      const currentDue = loan.dueDate ? parseDate(loan.dueDate) : calculateDueDate(loan.tanggal, loan.durasi);
      if (!currentDue) return;
      const newDue = new Date(currentDue);
      newDue.setDate(newDue.getDate() + 7);
      loan.durasi = Number(loan.durasi || 0) + 7;
      loan.dueDate = formatISODate(newDue);
      allLoans[session.email] = userLoans;
      saveLoans(allLoans);
      setLoanListMessage("Perpanjangan 7 hari berhasil diajukan.", "success");
      renderAll();
      return;
    }
...
// Approve / reject siswa pending (halaman verifikasi-siswa)
if (pendingList) {
  pendingList.addEventListener("click", (event) => {
    const approveBtn = event.target.closest("[data-approve-student]");
    const rejectBtn = event.target.closest("[data-reject-student]");
    if (!approveBtn && !rejectBtn) return;
    const session = getSession();
    if (!session || session.role !== "admin") {
      setMessage(pendingMessage, "Hanya admin yang dapat verifikasi.", "error");
      return;
    }
    const email = (approveBtn || rejectBtn).dataset.email;
    const users = getUsers();
    ...
    if (approveBtn) {
      ...
      saveUsers(users);
      setMessage(pendingMessage, "Akun siswa disetujui.", "success");
    } else {
      users.splice(idx, 1);
      saveUsers(users);
      setMessage(pendingMessage, "Pendaftaran siswa ditolak dan dihapus.", "success");
    }
    renderAll();
  });
}
...
if (signupForm) {
  signupForm.addEventListener("submit", (event) => {
    ...
    const gender = signupForm.elements["gender"]?.value.trim();
    const jurusan = signupForm.elements["jurusan"]?.value.trim();
    ...
    if (role === "student" && (!nis || !kelas || !gender || !jurusan)) {
      setMessage(signupMessage, "NIS, kelas, jenis kelamin, dan jurusan wajib diisi untuk siswa.", "error");
      return;
    }
    ...
    const newUser = {
      ...
      jurusan: role === "student" ? jurusan : null,
      email,
      password,
      role,
      status: role === "student" ? "pending" : "verified",
      verified_at: role === "student" ? null : new Date().toISOString(),
    };
    users.push(newUser);
    saveUsers(users);
    ...
    signupForm.reset();
    if (role === "student") {
      setMessage(signupMessage, "Pendaftaran berhasil. Menunggu verifikasi admin.", "success");
    } else {
      setMessage(signupMessage, "Akun admin dibuat. Silakan login.", "success");
    }
  });
}
...
if (loginForm) {
  loginForm.addEventListener("submit", (event) => {
    ...
    if (role === "admin") {
      ...
      if (!existingUser) {
      setMessage(loginMessage, "Akun belum terdaftar. Silakan sign up terlebih dahulu.", "error");
      return;
    }
    }
    ...
    // Blokir siswa yang belum diverifikasi
    if (userRole === "student" && existingUser.status && existingUser.status !== "verified") {
      setMessage(loginMessage, "Akun belum diverifikasi admin. Silakan tunggu persetujuan.", "error");
      return;
    }
